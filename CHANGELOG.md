...
- **Path-level residuals (27 alerts in 14 files)** — GHAS code-scanning ignores `// codeql[java/xss]` comments on or above the sink (same behavior documented for `java/ssrf` in PR #33). Each sink retains its runtime defense (typed JSON/XML DTO responses, reverse-proxy byte pass-through, `Encode.forHtml` / `plainTextError` at HTML-emitting boundaries), but the analyzer does not model the OWASP encoder or typed-media sinks (local model packs not loaded). The 14 residual files are added to `paths-ignore` in `.github/codeql/codeql-config.yml` with matching `suppressions.md` rows, per the PR #33/#35/#36 convention. Sink-line comments remain in code as documentation.
