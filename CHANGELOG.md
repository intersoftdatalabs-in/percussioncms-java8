...
- **DTD-URL SSRF guard** — `system/src/main/java/com/percussion/xml/PSDtdTree.java` now feeds the http/https URL branch through `URLValidation.validateURLString(...)` and rebuilds the URL with an explicit scheme literal before `openConnection()`. A `// codeql[java/ssrf]` comment on the line immediately above the sink closes alert #432.
...
- GHA rejects local `packs:` paths, so the in-repo `url-validation-ssrf` model pack is documentation only. Runtime `URLValidation` + scheme-literal rebuild + a `// codeql[java/ssrf]` comment on the line above each sink are the enforcement layer (trailing comments on the sink line were not honored).
