...
- **Editor decoration grid no longer forces legacy fixed pixels** (#2) — `WebUI/war/css/perc_decoration.css` switches `.vspan_*` and `.hspan_*` to `height: auto` / `width: auto` (no `!important`). Responsive customer themes (e.g. Cloud County CC `height: auto` / `width: auto`) now render sanely in editor + preview without a customer `!important` counter-override. Published default-theme sidebar/footer `min-height` floors (GH-757) live in `system/.../default_theme/theme.css` and are unchanged.
...
- **Editor / preview broken on responsive themes (8.1.7 `perc_decoration.css` !important)** (#2) — `perc_decoration.css` no longer forces `120/240/360/480px` vspan heights or `160/640/800/960px` hspan widths with `!important`. Customer themes that set `height/width: auto` on spans (e.g. `cccc-theme.css`) now render correctly in CMS chrome. Regression covered by new `VspanFooterAlignmentCssTest` (asserts decoration has no `!important` fixed grid, that theme.css still owns the published `min-height` floors for #757).
