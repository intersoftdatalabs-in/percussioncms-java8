...
- **DTD-URL SSRF guard** — `system/src/main/java/com/percussion/xml/PSDtdTree.java` now feeds the http/https URL branch through `URLValidation.validateURLString(...)` and rebuilds the URL with an explicit scheme literal before `openConnection()`. Sink-line `// codeql[java/ssrf]` closes alert #432.
- **Feed metadata-service SSRF guard** — `deliverytiersuite/delivery-tier-suite/feeds/src/main/java/com/percussion/delivery/feeds/services/PSFeedService.java` runs the constructed metadata-service URL through `URLValidation.validateURLString(...)` and rebuilds the URI with an explicit scheme literal before `client.target(url + ...)`. Sink-line `// codeql[java/ssrf]` closes alert #431.
...
- The `url-validation-ssrf` CodeQL model pack from PR #9 (`codeql/port-shared-guards`) is already in `.github/codeql/models/models/` so the `URLValidation` barrier annotation is in effect for the Advanced analyzer; no model changes required.
- CodeQL alerts #431 (`PSFeedService.java:461`) and #432 (`PSDtdTree.java:204`) are the only `java/ssrf` Critical+High alerts on `main` as of 2026-08-12.
